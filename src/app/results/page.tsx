        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <CardDescription className="text-xs">{result.bank || result.provider}</CardDescription>
            </div>
          </div>
          <div className="text-right">
            ...
            <div className="text-xs text-gray-500">回饋率</div>
          </div>
        </div>
      </CardHeader>
      <CardContent className="p-4 pt-0">
        ...
        <div className="flex flex-wrap gap-1 mb-3">
          {result.features.map((feature: string, index: number) => (
            <Badge key={index} className="text-xs px-2 py-0.5 bg-green-100 text-green-700 hover:bg-green-200">
              {feature}
            </Badge>
          ))}
        </div>
  ...
  return (
    <div className="min-h-screen bg-white py-8">
      <div className="container mx-auto px-4">
        ...
        <div className="space-y-4 mb-8">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button variant="outline" size="sm" className="text-xs border-green-200 text-green-700 hover:bg-green-50">回饋率</Button>
              <Button variant="outline" size="sm" className="text-xs border-green-200 text-green-700 hover:bg-green-50">評分</Button>
            </div>
          </div>
        ...
        {/* 頁尾 */}
        <div className="text-center text-sm text-gray-500 border-t border-green-100 pt-8">
          <p>📋 資料僅供參考，實際回饋以銀行公告為準</p>
          <p className="mt-2">最後更新：{new Date().toLocaleDateString('zh-TW')}</p>
        </div>
      </div>
    </div>
  )
